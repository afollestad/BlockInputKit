...
- **Inline code pairs across line endings, like CommonMark, and never wraps mid-span.** A closer stranded on the next line used to open a bogus span that shifted every later pairing. The break rule lives on `BlockInputDelimiterGlyphs`, the one layout-manager delegate rendering and measurement share, so a span wraps at the same place in both.
