...
extension BlockInputTextView {
    ...
    /// Keep valid bounds for bidirectional runs; glyph positions recover the empty rectangles.
    func drawInlineCodeBackgrounds(in dirtyRect: NSRect) {
        ...
        textStorage.enumerateAttribute(.blockInputInlineCodeBackground, in: fullRange) { value, range, _ in
            ...
            while glyphIndex < NSMaxRange(glyphRange) {
                ...
                var rect = layoutManager.boundingRect(forGlyphRange: intersection, in: textContainer)
                if rect.width <= 0 {
                    ...
                    let endX = endIndex < NSMaxRange(lineRange)
                        ...
                        : lineRect.maxX
                    rect.origin.x = min(startX, endX)
                    rect.size.width = abs(endX - startX)
                }
                rect.origin.y = lineRect.minY
