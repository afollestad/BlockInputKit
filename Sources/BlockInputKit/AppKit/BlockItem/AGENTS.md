...
- Read `BlockInputTextStyle.lineSpacing` only through `BlockInputBlockItem.lineSpacing(for:style:)`; rendering, typing attributes, and every height path must agree on which kinds carry it or offscreen measurement drifts from what a mounted row lays out.
- Center line-aligned chrome — inline chip fills, list markers, the quote bar — on `NSLayoutManager.blockInputGlyphLineRect(forGlyphAt:)`, never on the used rect or a bounding rect, both of which carry the trailing line spacing and sink the chrome below its own glyphs.
